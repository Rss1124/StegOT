...
from fightingcv_attention.conv.CondConv import CondConv
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        qkv = self.qkv(x).reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(2,
                                                                                         0, 3, 1,
                                                                                         4).contiguous()  # (3,B*num_windows,heads,N,head_dim)
        ...
        if mask is not None:
            ...
            attn = attn.view(B_ // nW, nW, self.num_heads, N, N) + mask.unsqueeze(1).unsqueeze(
                0)  # att(bs,nW,heads,N,N); mask(1,nW,1,N,N)
...
def PatchReverse(x, patch_size):
    ...
    x = rearrange(x, 'b (p1 p2) (ph pw c) -> b c (p1 ph) (p2 pw)', p1=p1, p2=p2, ph=patch_size,
                  pw=patch_size).contiguous()
    return x
...
class GEB(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, patch_size, depth,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0.,
                 norm_layer=nn.LayerNorm, use_checkpoint=False, drop_key=False, mode=None):
        super().__init__()
        self.mode = mode
    ...
    def forward(self, pool3, latent):
        map = OT_MAPPING()
        x = map(pool3, latent, self.mode)  # x (8, 128, 16, 16)
        for param in map.parameters():
            param.requires_grad = False
        x = PatchDivide(x, self.patch_size)
        B_, N, C = x.shape
        x = rearrange(x, 'b (h w) c -> b c h w ', h=int(N ** 0.5), w=int(N ** 0.5)).contiguous()  # 转换为图像表示 (8,1024,8,8)
...
class Channel_Adaptive_Transformer_Block(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size, patch_size,
                 ...
                 fused_window_process=False, drop_key=False):
        super().__init__()
        self.transformer_block = Swin_Transformer(dim, to_2tuple(input_resolution), depth, num_heads, window_size,
                                                  mlp_ratio, qkv_bias,
                                                  qk_scale, drop, attn_drop, drop_path, norm_layer, use_checkpoint,
                                                  fused_window_process, drop_key=drop_key)
...
class CATB_Layer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size, patch_size, depth=1,
        ...
        self.use_checkpoint = use_checkpoint
        self.blocks = nn.ModuleList([
            Channel_Adaptive_Transformer_Block(dim, input_resolution, num_heads, window_size, patch_size, depth_tr,
                                               mlp_ratio, qkv_bias,
                                               qk_scale, drop, attn_drop, drop_path, norm_layer, use_checkpoint,
                                               fused_window_process, drop_key) for i in range(depth)])
...
class OT_MAPPING(nn.Module):
    """ ot map"""

    def __init__(self):
        super().__init__()
        self.ot_emd = ot.da.EMDTransport()

    def forward(self, pool3, latent, mode):  # bottle (8,128,16,16)
        maps = []
        N, C, H, W = latent.shape
        latent = latent.reshape(N, C, H * W)
        white_noise = torch.randn(N, C, H * W).to('cuda:0')  # source (128,256)
        bottle = torch.zeros_like(latent)

        for i in range(N):
            self.ot_emd.fit(Xs=white_noise[i], Xt=latent[i])
            maps.append(self.ot_emd)

        for index, map in enumerate(maps):
            bottle[index] = map.transform(Xs=white_noise[index])
            if mode == 'reveal':
                bottle[index] += white_noise[index]

        bottle = bottle.reshape(N, C, H, W)
        return bottle
...
class StegFormer(nn.Module):
    ...
    def __init__(self, img_resolution, input_dim=3, output_dim=3, cnn_emb_dim=16, output_act=None, patch_size=2,
                 num_heads=[1, 2, 4, 8, 16, 16, 8, 4, 2], window_size=8,
                 depth=[1, 1, 1, 1, 2, 1, 1, 1, 1], depth_tr=[2, 2, 2, 2, 2, 2, 2, 2], mlp_ratio=4., qkv_bias=True,
                 qk_scale=None, drop=0., attn_drop=0., mode=None,
                 drop_path=0., norm_layer=nn.LayerNorm, use_checkpoint=False, drop_key=False, pretrain_encoder=None):
        super().__init__()
        ...
        self.encoderlayer_0 = CATB_Layer(self.token_dim, self.patch_resolution, num_heads[0],
                                         window_size, patch_size, depth[0], depth_tr[0], mlp_ratio, qkv_bias, qk_scale,
                                         drop, attn_drop, drop_path, norm_layer, use_checkpoint, drop_key=drop_key)
        ...
        self.encoderlayer_1 = CATB_Layer(self.token_dim * 2, self.patch_resolution // (2 ** 1),
                                         num_heads[1], window_size, patch_size, depth[1], depth_tr[1], mlp_ratio,
                                         qkv_bias, qk_scale, drop, attn_drop, drop_path, norm_layer, use_checkpoint,
                                         drop_key=drop_key)
        ...
        self.encoderlayer_2 = CATB_Layer(self.token_dim * 4, self.patch_resolution // (2 ** 2),
                                         num_heads[2], window_size, patch_size, depth[2], depth_tr[2], mlp_ratio,
                                         qkv_bias, qk_scale, drop, attn_drop, drop_path, norm_layer, use_checkpoint,
                                         drop_key=drop_key)
        ...
        self.encoderlayer_3 = CATB_Layer(self.token_dim * 8, self.patch_resolution // (2 ** 3),
                                         num_heads[3], window_size, patch_size, depth[3], depth_tr[3], mlp_ratio,
                                         qkv_bias, qk_scale, drop, attn_drop, drop_path, norm_layer, use_checkpoint,
                                         drop_key=drop_key)
        ...
        self.bottleneck = GEB(self.token_dim * 16, self.patch_resolution // (2 ** 4),
                              num_heads[4], patch_size, depth[4], mlp_ratio, qkv_bias, qk_scale, drop, attn_drop,
                              drop_path, norm_layer, use_checkpoint, mode=mode, drop_key=drop_key)
        ...
        self.decoderlayer_0 = CATB_Layer(self.token_dim * 16, self.patch_resolution // (2 ** 3),
                                         num_heads[5], window_size, patch_size, depth[5], depth_tr[4], mlp_ratio,
                                         qkv_bias, qk_scale, drop, attn_drop, drop_path, norm_layer, use_checkpoint,
                                         drop_key=drop_key)
        ...
        self.decoderlayer_1 = CATB_Layer(self.token_dim * 8, self.patch_resolution // (2 ** 2),
                                         num_heads[6], window_size, patch_size, depth[6], depth_tr[5], mlp_ratio,
                                         qkv_bias, qk_scale, drop, attn_drop, drop_path, norm_layer, use_checkpoint,
                                         drop_key=drop_key)
        ...
        self.decoderlayer_2 = CATB_Layer(self.token_dim * 4, self.patch_resolution // (2 ** 1),
                                         num_heads[7], window_size, patch_size, depth[7], depth_tr[6], mlp_ratio,
                                         qkv_bias, qk_scale, drop, attn_drop, drop_path, norm_layer, use_checkpoint,
                                         drop_key=drop_key)

        self.upsampler_3 = UpSampler(self.dim * 4, self.dim)
        self.decoderlayer_3 = CATB_Layer(self.token_dim * 2, self.patch_resolution,
                                         num_heads[8], window_size, patch_size, depth[8], depth_tr[7], mlp_ratio,
                                         qkv_bias, qk_scale, drop, attn_drop, drop_path, norm_layer, use_checkpoint,
                                         drop_key=drop_key)
    ...
    def forward(self, x):
        # if self.mode == 'hide':
        #     latent = self.pretrain_encoder(x)
        # elif self.mode == 'reveal' and auxiliary is not None:
        #     latent = auxiliary
        # else:
        #     print("input error")
        latent = self.pretrain_encoder(x)
        x = self.embedding(x)  # (8,256,256)
        ...
        # bottle neck
        bottle = self.bottleneck(pool3, latent)  # (128,16,16)
